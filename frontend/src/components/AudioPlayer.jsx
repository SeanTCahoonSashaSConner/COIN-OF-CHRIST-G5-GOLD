...
export default function AudioPlayer() {
  ...
  // 2. Audio Playback & 20s Verification Loop
  useEffect(() => {
    let interval = null;

    if (isPlaying && !isWindowOpen && !cooldownStartTime) {
      interval = setInterval(() => {
        // Track current audio position for strict resume
        if (audioRef.current) {
          setSavedAudioPosition(audioRef.current.currentTime);
        }

        // Increment total 33-hr listening tally
        setTotalListenedSecs((prevTotal) => {
          const newTotal = prevTotal + 1;

          // Check if 33-hour Cap Hit
          if (newTotal >= CAP_33_HOURS) {
        ...
        // Track 20-second verification trigger
        setCyclePlaytime((prevCycle) => {
  ...
  // Toggle Play/Stop (Resumes at saved position)
  const togglePlay = () => {
    if (cooldownStartTime) return; // Locked out during 48-hr window
    ...
    if (isPlaying) {
      ...
      // Restore exact timestamp before playing
  ...
  // Lock Seeker (Anti-Rewind / Anti-Fast-Forward)
  const handleTimeUpdate = () => {
    if (!audioRef.current) return;
    // Prevent seeking ahead or behind saved state manually
  ...
  return (
    <div className="min-h-screen bg-black text-amber-400 p-4 flex flex-col items-center justify-between font-mono select-none">
      ...
      {/* Audio Element linked to Anti-Tamper GPSDO enforcement */}
      <audio 
        ref={audioRef} 
        src={bibleAudioSrc} 
        preload="auto"
      ...
      {/* 48-Hour Commercial & Spending Countdown Notice */}
      {cooldownStartTime && (
        <div className="w-full max-w-sm bg-red-950/30 border border-red-700/60 rounded-lg p-3 text-center my-1 animate-pulse">
          ...
          <div className="text-[9px] text-red-400/80 mt-1">
            Spend earned tokens & watch merchant offers. Audio resumes automatically after 48 hours.
          </div>
        </div>
      )}

      {/* Atomic Clock & GPSDO Tracking */}
